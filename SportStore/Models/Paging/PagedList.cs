...
namespace SportStore.Models.Paging
{
    public class PagedList<T> : List<T>
    {
        public PagedList(IQueryable<T> query, QueryOptions options = null)
        {
            ...
            this.AddRange(query.Skip((CurrentPage - 1) * PageSize).Take(PageSize));
        ...
        public IQueryable<T> Search(IQueryable<T> query, string searchPropertyName, string searchTerm)
        {
        ...
        public IQueryable<T> Order(IQueryable<T> query, string propertyName, bool desc)
        {
            var parameter = Expression.Parameter(typeof(T), "x");
